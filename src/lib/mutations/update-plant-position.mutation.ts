...
export function useUpdatePlantMutation(cmaClient: PlainClientAPI) {
  ...
  return useMutation<EntryProps<KeyValueMap>, Error, Plant, Plant[]>({
    mutationFn: updatePlantPosition,
    onMutate: async (newPlant) => {
      await queryClient.cancelQueries({
        queryKey: [plantsWithPositionQueryKey],
      });

      const previousPlants = queryClient.getQueryData<Plant[]>([
